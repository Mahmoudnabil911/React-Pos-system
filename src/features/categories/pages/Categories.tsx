...
import { MOCK_CATEGORIES, statusBadgeClass } from '../../../utils/mockData';
...
export default function Categories() {
  ...
  const columns = [
    { key: 'name', label: t('common.name'), render: (row: typeof MOCK_CATEGORIES[0]) => <span style={{ fontWeight: 600 }}>{row.name}</span> },
    { key: 'description', label: t('common.description') },
    { key: 'products', label: t('nav.products'), render: (row: typeof MOCK_CATEGORIES[0]) => <span className="badge badge-primary">{row.products}</span> },
    { key: 'status', label: t('common.status'), render: (row: typeof MOCK_CATEGORIES[0]) => <span className={statusBadgeClass(row.status)}>{t(`common.${row.status.toLowerCase()}`) || row.status}</span> },
  ...
  return (
    <div>
      <PageHeader
        title={t('nav.categories')}         subtitle={t('categories.subtitle')}
        breadcrumbs={[{ label: t('nav.dashboard'), route: '/dashboard' }, { label: t('nav.inventoryGroup') }, { label: t('nav.categories') }]}
      >
        <button className="btn-primary" onClick={handleCreate}><svg style={{ width: '16px', height: '16px' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>{t('categories.addCategory')}</button>
      </PageHeader>
      <DataTable columns={columns as never} data={MOCK_CATEGORIES as never} isLoading={isLoading} searchPlaceholder={t('categories.search')} pageSize={10} />
    
      <Modal 
        isOpen={isModalOpen} 
        onClose={() => setIsModalOpen(false)} 
        title={editingItem ? t('categories.editCategory') : t('categories.addCategory')}
        footer={
      ...
      >
        <form id="categoryForm" onSubmit={handleSubmit} className="flex flex-col gap-4">
